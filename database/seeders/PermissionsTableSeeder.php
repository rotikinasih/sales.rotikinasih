<?php

namespace Database\Seeders;

use Illuminate\Database\Seeder;
use Spatie\Permission\Models\Permission;

class PermissionsTableSeeder extends Seeder
{
    /**
     * Run the database seeds.
     *
     * @return void
     */
    public function run()
    {
        //permission dashboard
        Permission::create(['name' => 'dashboard.index', 'guard_name' => 'web']);

        //permission PT
        Permission::create(['name' => 'pt.index', 'guard_name' => 'web']);
        Permission::create(['name' => 'pt.create', 'guard_name' => 'web']);
        Permission::create(['name' => 'pt.edit', 'guard_name' => 'web']);

        //permission divisi
        Permission::create(['name' => 'divisi.index', 'guard_name' => 'web']);
        Permission::create(['name' => 'divisi.create', 'guard_name' => 'web']);
        Permission::create(['name' => 'divisi.edit', 'guard_name' => 'web']);

        //permission jabatan
        Permission::create(['name' => 'jabatan.index', 'guard_name' => 'web']);
        Permission::create(['name' => 'jabatan.create', 'guard_name' => 'web']);
        Permission::create(['name' => 'jabatan.edit', 'guard_name' => 'web']);

         //permission jabatan
        Permission::create(['name' => 'posisi.index', 'guard_name' => 'web']);
        Permission::create(['name' => 'posisi.create', 'guard_name' => 'web']);
        Permission::create(['name' => 'posisi.edit', 'guard_name' => 'web']);

        //permission karyawan
        Permission::create(['name' => 'karyawan.index', 'guard_name' => 'web']);
        Permission::create(['name' => 'karyawan.create', 'guard_name' => 'web']);
        Permission::create(['name' => 'karyawan.edit', 'guard_name' => 'web']);
        Permission::create(['name' => 'karyawan.delete', 'guard_name' => 'web']);
        Permission::create(['name' => 'karyawan.detail', 'guard_name' => 'web']);

        //permission roles
        Permission::create(['name' => 'roles.index', 'guard_name' => 'web']);
        Permission::create(['name' => 'roles.create', 'guard_name' => 'web']);
        Permission::create(['name' => 'roles.edit', 'guard_name' => 'web']);
        Permission::create(['name' => 'roles.delete', 'guard_name' => 'web']);

        //permission permission
        Permission::create(['name' => 'permissions.index', 'guard_name' => 'web']);
        Permission::create(['name' => 'permissions.edit', 'guard_name' => 'web']);

        //permission users
        Permission::create(['name' => 'users.index', 'guard_name' => 'web']);
        Permission::create(['name' => 'users.create', 'guard_name' => 'web']);
        Permission::create(['name' => 'users.edit', 'guard_name' => 'web']);

        Permission::create(['name' => 'apps.organisasi.indexAll', 'guard_name' => 'web']);
        Permission::create(['name' => 'apps.pelanggaran.indexAll', 'guard_name' => 'web']);

        Permission::create(['name' => 'apps.resign.index', 'guard_name' => 'web']);
        Permission::create(['name' => 'apps.resign.create', 'guard_name' => 'web']);
        Permission::create(['name' => 'apps.resign.edit', 'guard_name' => 'web']);

        Permission::create(['name' => 'apps.phk.index', 'guard_name' => 'web']);
        Permission::create(['name' => 'apps.phk.edit', 'guard_name' => 'web']);
        Permission::create(['name' => 'apps.phk.create', 'guard_name' => 'web']);

        Permission::create(['name' => 'apps.pelatihan.index', 'guard_name' => 'web']);
        Permission::create(['name' => 'apps.pelatihan.create', 'guard_name' => 'web']);
        Permission::create(['name' => 'apps.pelatihan.edit', 'guard_name' => 'web']);

    }
}
